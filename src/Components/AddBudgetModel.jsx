...
function AddBudgetModel({ isOpen ,setIsOpen }) {
    const nameRef = useRef();
    const MaxSpendingRef = useRef();
    const [budgetValues, setBudgetValues] = useState({ name: nameRef.current, maxSpending: MaxSpendingRef.current });
        ...
        <div>
            <div className="model w-[95%] shadow-xl bg-white  border-black absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] rounded-lg z-10">
                <div className="header border-b-[1px] px-2 border-gray-400 flex justify-between items-center">
                    <h1 className="text-lg font-semibold">Add Budget</h1>
                ...
                <form className="p-2 flex flex-col items-center justify-center gap-2">
                    ...
                    <button className="border-[2px] bg-blue-500 rounded-lg px-4 py-2 cursor-pointer text-white" type="submit" onClick={handleAddBudget}>Add Budget</button>
                </form>
            </div>
